...
class Point(object):
    ...
    def move_to(self, x, y):
        p1 = Point(x,y)
        self.distance_traveled = self.distance_traveled + self.get_distance_from(p1)
        self.x = x
        self.y = y
        self.count += 1

    def move_by(self, dx, dy):
        self.distance_traveled = self.distance_traveled + math.sqrt(((dy) ** 2 + (dx) ** 2))
        self.x = self.x +dx
        self.y = self.y +dy
        self.count += 1
    ...
    def get_distance_from_start(self):
        cent = Point(self.centx, self.centy)
        return self.get_distance_from(cent)
    ...
    def halfway_to(self, p2):
        p3 = ((p2.x+self.x)/2,(p2.y+self.y)/2)
        return p3
...
def run_test_halfway_to():
    ...
    """
    # ------------------------------------------------------------------
    # Doneso: 13.  Follow the same instructions as in TO-DO 3 above,
